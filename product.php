...
Hello  <?=$login->login()?> <a href="?deco">logout</a>
 ...
 <div>
  <a href="addProduct.php?cat=<?=$_GET["cat"]?>">Add a Product</a>
</div>
<?php 
  $dbCon= new Connection();
  $conn=$dbCon->OpenCon();
  $sql = "SELECT * FROM `product` WHERE product.Category_id=".$_GET["cat"] ;
   ...
   <?php if (mysqli_num_rows($result) > 0) {
        // output data of each row
            while($row = mysqli_fetch_assoc($result)) {?>
                ...
                <td> <?=$row["Price"]?></td>
                <td> <?=$row["Stock"]?></td>
                <td> <a href="addProduct.php?product=<?=$row["Product_id"]?>&cat=<?=$_GET["cat"]?>"><i class="bi bi-pencil-fill"></i></a></td>
                <td> <a href="deleteProduct.php?product=<?=$row["Product_id"]?>&cat=<?=$_GET["cat"]?>"><i class="bi bi-trash"></i></a></td>
